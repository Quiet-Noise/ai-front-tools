...
export const ChatEmbed: React.FC<ChatEmbedProps> = ({
  ...
  const adjustTextareaHeight = useCallback(() => {
    ...
    // Calculate new height (min 36px, max 120px)
    const scrollHeight = textarea.scrollHeight;
    const newHeight = Math.min(Math.max(scrollHeight, 36), 120);
    textarea.style.height = `${newHeight}px`;

    // Hide scrollbar if content fits or is empty, show only when it overflows max height
    if (scrollHeight <= 120 || !textarea.value) {
  ...
  useEffect(() => {
    if (
      mergedConfig.enableMessageSuggestions &&
      mergedConfig.initialSuggestions &&
      mergedConfig.initialSuggestions.length > 0 &&
      messages.length === 0
    ) {
      setCurrentSuggestions(
        mergedConfig.initialSuggestions.slice(0, mergedConfig.maxSuggestions),
      );
    ...
    try {
      ...
      for (let i = 0; i < filesToSend.length; i++) {
        ...
        if (
          ...
          setCurrentSuggestions(
            response.suggestions.slice(0, mergedConfig.maxSuggestions),
          );
          setShowSuggestions(true);
        }
      }
      ...
      try {
        if (navigator.permissions) {
          ...
          if (cameraPermission.state === "denied") {
            alert(
              "Camera access is blocked in Arc. Please:\n1. Click the shield icon in the address bar\n2. Allow camera access\n3. Refresh the page",
            );
            return;
          }
        }
      ...
      if (navigator.permissions) {
        try {
          ...
          if (permission.state === "denied") {
            alert(
              "Camera permission is denied. Please enable camera access in your browser settings and reload the page.",
            );
            return;
          }
        } catch (permError) {
      ...
      try {
        if (navigator.permissions) {
          ...
          if (micPermission.state === "denied") {
            alert(
              "Microphone access is blocked in Arc. Please:\n1. Click the shield icon in the address bar\n2. Allow microphone access\n3. Refresh the page",
            );
            return;
          }
        }
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          if (
            ...
            setCurrentSuggestions(
              suggestions.slice(0, mergedConfig.maxSuggestions),
            );
            setShowSuggestions(true);
          }
        } catch (error) {
  ...
  if (shouldShowToggleButton) {
    return (
      <>
        {cameraModal}
        <div
          className={`chat-widget-container chat-widget-container--${mergedConfig.position}${!isOpen ? " chat-closed" : ""}`}
        >
            ...
            >
              <div className="chat-embed__header">
                <div className="chat-embed__header-content">
                  ...
                  {mergedConfig.subtitle && (
                    <p className="chat-embed__subtitle">
                      {mergedConfig.subtitle}
                    </p>
                  )}
                </div>
              ...
              {mergedConfig.enableUserInfo && showUserForm ? (
                <UserInfoForm
                  ...
                  isLoading={false}
                  requiredFields={
                    (mergedConfig.userInfoRequiredFields as (
                      | "name"
                      | "email"
                      | "phone"
                    )[]) || ([] as const)
                  ...
                  <div className="chat-embed__input-area">
                    ...
                    {mergedConfig.enableMessageSuggestions &&
                      ...
                      currentSuggestions.length > 0 && (
                        <MessageSuggestions
                          suggestions={currentSuggestions.map(
                            (text, index) => ({
                              id: `suggestion-${index}`,
                              text,
                              type:
                                messages.length === 0 ? "starter" : "followup",
                            }),
                          )}
                    ...
                    <div
                      className={`chat-embed__input-row ${isRecording ? "chat-embed__input-row--recording" : ""}`}
                    >
                      {isRecording ? (
                        <>
                        ...
                        <>
                          ...
                          {inputValue.trim() || pendingFiles.length > 0 ? (
                            ...
                            >
                              <Send
                                size={18}
                                className="chat-embed__send-icon"
                              />
                            </button>
                          ) : (
                            <>
